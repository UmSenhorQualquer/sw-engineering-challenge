...
def test_bloq_fixture(client, test_data):
...
def test_get_lockers(client, test_data, test_bloq):
    # Create test locker
...
def test_get_locker(client, test_data, test_bloq):
    # Create test locker
    idx = random.randint(0, len(test_data["lockers"]) - 1)
    locker_data = test_data["lockers"][idx]
    response = client.post("/lockers/", json=locker_data)
    ...
    # Get specific locker
    response = client.get(f"/lockers/{locker_data['id']}")
    assert response.status_code == 200
    assert response.json()["id"] == locker_data["id"]
    
    # Test non-existent locker
    response = client.get("/lockers/nonexistent")
    assert response.status_code == 404

def test_create_locker(client, test_data, test_bloq):
    # Create locker
    idx = random.randint(0, len(test_data["lockers"]) - 1)
    locker_data = test_data["lockers"][idx]
    del locker_data["id"]
...
def test_update_locker_status(client, test_data, test_bloq):
    # Create test locker
    idx = random.randint(0, len(test_data["lockers"]) - 1)
    ...
    new_status = "OPEN"
    response = client.put(f"/lockers/{locker_id}/status?status={new_status}")
    assert response.status_code == 200
    assert response.json()["status"] == new_status
    
    # Test invalid status
    response = client.put(f"/lockers/{locker_id}/status?status=INVALID")
    assert response.status_code == 422  # Validation error
    
    # Test non-existent locker
    response = client.put("/lockers/nonexistent/status?status=OPEN")
    assert response.status_code == 404 