...
@pytest.mark.asyncio
async def test_get_rents(client, test_data, test_locker):
    # Create test rent
    idx = random.randint(0, len(test_data["rents"]) - 1)
    rent_data = test_data["rents"][idx]
    response = await client.post("/rents/", json=rent_data)
    assert response.status_code == 200
    
    # Get all rents
    response = await client.get("/rents/")
    assert response.status_code == 200
    assert len(response.json()) == 1
    assert response.json()[0]["id"] == rent_data["id"]

@pytest.mark.asyncio
async def test_get_rent(client, test_data, test_locker):
    # Create test rent
    idx = random.randint(0, len(test_data["rents"]) - 1)
    rent_data = test_data["rents"][idx]
    response = await client.post("/rents/", json=rent_data)
    ...
    # Get specific rent
    response = await client.get(f"/rents/{rent_data['id']}")
    assert response.status_code == 200
    assert response.json()["id"] == rent_data["id"]
    
    # Test non-existent rent
    response = await client.get("/rents/nonexistent")
    assert response.status_code == 404

@pytest.mark.asyncio
async def test_create_rent(client, test_data, test_locker):
    # Create rent
    idx = random.randint(0, len(test_data["rents"]) - 1)
    rent_data = test_data["rents"][idx]
    del rent_data["id"] 
...
@pytest.mark.asyncio
async def test_update_rent_status(client, test_data, test_locker):
    # Create test rent
    idx = random.randint(0, len(test_data["rents"]) - 1)
    rent_data = test_data["rents"][idx]
    del rent_data["id"]
    ...
    response = await client.put(f"/rents/{rent_data['id']}/status?status={new_status}")
    assert response.status_code == 200
    assert response.json()["status"] == new_status
    
    # Test invalid status
    response = await client.put(f"/rents/{rent_data['id']}/status?status=INVALID")
    assert response.status_code == 422  # Validation error
    
    # Test non-existent rent
    response = await client.put("/rents/nonexistent/status?status=WAITING_PICKUP")
    assert response.status_code == 404

@pytest.mark.asyncio
async def test_rent_lifecycle(client, test_data, test_locker):
    ...
    response = await client.put(f"/rents/{rent_id}/status?status=WAITING_DROPOFF")
    assert response.status_code == 200
    assert response.json()["status"] == "WAITING_DROPOFF"
    
    # Update to WAITING_PICKUP
    response = await client.put(f"/rents/{rent_id}/status?status=WAITING_PICKUP")
    assert response.status_code == 200
    assert response.json()["status"] == "WAITING_PICKUP"
    
    # Finally update to DELIVERED
    response = await client.put(f"/rents/{rent_id}/status?status=DELIVERED")
    assert response.status_code == 200
